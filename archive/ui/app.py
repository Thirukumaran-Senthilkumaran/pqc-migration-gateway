...
def normalize_api_url(url: str) -> str:
    """Use loopback IPv4 — avoids errno 99 / IPv6 issues with localhost."""
    u = url.strip().rstrip("/")
...
def default_api_url() -> str:
    try:
        ...
        if hasattr(st, "secrets") and "API_BASE_URL" in st.secrets:
            return normalize_api_url(str(st.secrets["API_BASE_URL"]))
    except Exception:
        pass
    return normalize_api_url(
        os.getenv("API_BASE_URL", os.getenv("PQCG_API_URL", "http://127.0.0.1:8000"))
    )


def is_loopback_api(url: str) -> bool:
...
def use_embedded_api() -> bool:
    ...
    if mode == "remote":
        return False
    return bool(st.session_state.get("use_embedded_fallback"))


def _enable_embedded() -> None:
    st.session_state["use_embedded_fallback"] = True


def _remote_error_should_fallback(err: Exception) -> bool:
    msg = str(err).lower()
    return any(
        x in msg
        for x in ("99", "cannot assign", "addrnotavail", "connection refused", "failed to connect", "connect error")
    )


def get_api() -> str:
    override = st.session_state.get("api_url_override", "").strip()
    return normalize_api_url(override) if override else default_api_url()
...
def _direct_get(path: str):
    try:
        from ui.direct_api import direct_get
    except ImportError:
        from direct_api import direct_get
    return direct_get(path)


def _direct_post(path: str, body: dict):
    try:
        from ui.direct_api import direct_post
    except ImportError:
        from direct_api import direct_post
    return direct_post(path, body)


def api_get(path: str):
    if use_embedded_api():
        try:
            return _direct_get(path)
        except Exception as e:
            st.error(f"Built-in API error: {e}")
            return None
    ...
    try:
        ...
        if _remote_error_should_fallback(e):
            _enable_embedded()
            st.info("Remote API unreachable — switched to **built-in mode** (works without a separate API server).")
            return api_get(path)
...
def api_post(path: str, body: dict):
    ...
    try:
        ...
        st.error(f"API error (`{api}`): {e}")
        _api_troubleshoot(e)
        return None


def api_get_report(fmt: str):
    path = f"/api/reports/{fmt}"
    if use_embedded_api():
        result = _direct_get(path)
        return result["content"], result["binary"]
...
def _api_troubleshoot(err: Exception) -> None:
    msg = str(err).lower()
    api = get_api()
    if "99" in msg or "cannot assign" in msg or "addrnotavail" in msg:
        st.warning(
            "The UI cannot reach the API at that address. "
            "Use `http://127.0.0.1:8000` locally — not your Wi‑Fi IP (`192.168.x.x`) "
            "or public IP. Change it in the sidebar and click **Apply**."
        )
    elif "connection refused" in msg or "failed to connect" in msg:
        st.warning(
            "The API server is not running. Open a terminal and run:\n\n"
            "`python -m uvicorn backend.main:app --host 127.0.0.1 --port 8000`"
        )
    elif api.startswith("http://127.0.0.1") or api.startswith("http://localhost"):
        st.caption("Local mode: start the API with `start.bat` or the uvicorn command above.")
...
4. Run: `python connector.py --token <token> --url <cloud-api-url>`
    ...
    st.code(f"python connector.py --token YOUR_TOKEN --url {get_api()}", language="bash")
...
def page_reports():
    ...
    for i, (fname, label, fmt) in enumerate(reports):
        with cols[i % 3]:
            content, is_binary = api_get_report(fmt)
    ...
    with st.sidebar:
        ...
        if ok:
            if "built-in" in detail:
                st.success("Built-in API (no separate server needed)")
            else:
                st.success("API connected")
        else:
            st.error("API offline")
            st.caption(detail)
        st.caption(f"URL: `{api}`")
        with st.expander("API connection settings"):
            mode = st.radio(
                "API mode",
                ["Auto (recommended)", "Built-in only", "Remote API only"],
                index={"embedded": 1, "remote": 2}.get(st.session_state.get("api_mode"), 0),
                help="Built-in works on Streamlit Cloud without Render. Remote needs a deployed API.",
            )
            if mode == "Built-in only":
                st.session_state["api_mode"] = "embedded"
                st.session_state.pop("use_embedded_fallback", None)
            elif mode == "Remote API only":
                st.session_state["api_mode"] = "remote"
                st.session_state.pop("use_embedded_fallback", None)
            else:
            ...
            new_url = st.text_input(
                "Remote API base URL",
                value=st.session_state.get("api_url_override") or default_api_url(),
                placeholder="https://your-api.onrender.com",
            )
            if st.button("Apply API URL", type="primary"):
                st.session_state["api_url_override"] = normalize_api_url(new_url)
                st.rerun()
            st.caption("Built-in: no setup · Remote: deploy API on Render first")
