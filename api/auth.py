"""Connector token authentication (Bearer)."""
...
def generate_token() -> tuple[str, str, str]:
    """Return (raw_token, sha256_hash, display_prefix)."""
    raw = f"pqcg_{secrets.token_urlsafe(32)}"
    return raw, hash_token(raw), raw[:14]
...
def create_connector_token(db: Session, name: str, org_name: str = "Default Org") -> tuple[ConnectorToken, str]:
    ...
    row = ConnectorToken(name=name, token_hash=token_hash, token_prefix=prefix, org_name=org_name)
    db.add(row)
    db.commit()
    db.refresh(row)
    return row, raw
...
def verify_connector_token(
    ...
    row = (
        ...
        .filter(ConnectorToken.token_hash == hash_token(creds.credentials), ConnectorToken.active.is_(True))
        .first()
    )
    if not row:
